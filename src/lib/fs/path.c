...
static int
process_glob_aux(const char *pattern, int start_sep, int end_sep,
                 int last_char, int separator, smartlist_t *results,
                 int add_no_glob, unglob_fn unglob, handle_glob_fn handle_glob,
                 glob_predicate_fn should_append,
                 glob_predicate_fn should_recurse)
{
  ...
  size_t glob_size = last_char && !separator ? end_sep + 1 : end_sep;
  ...
  // and call recursively to get all paths
  int error_found = 0;
  const char *pattern_after = &pattern[end_sep+1];
  SMARTLIST_FOREACH_BEGIN(unglobbed_paths, char *, current_path) {
    ...
    } else if (!(last_char && separator && file_type != FN_DIR)) {
...
process_glob(const char *pattern, int add_no_glob, unglob_fn unglob,
             ...
             glob_predicate_fn should_recurse)
{
  int i, start_sep = -1, end_sep = -1, glob_found = 0, last_char, separator;
  ...
  for (i = 0; pattern[i]; i++) {
    ...
    separator = pattern[i] == *PATH_SEPARATOR;
#ifdef _WIN32
    separator = separator || pattern[i] == '/';
#endif
  ...
  if (process_glob_aux(pattern, start_sep, end_sep, last_char, separator,
                       results, add_no_glob, unglob, handle_glob,
                       should_append, should_recurse) < 0) {
